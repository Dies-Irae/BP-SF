...
cimport numpy as np
from myldpc.bp_decoder._bp_decoder cimport (
    BpSparse, BpDecoderCpp, NULL_INT_VECTOR,
    PRODUCT_SUM, MINIMUM_SUM,
    SYNDROME, RECEIVED_VECTOR, AUTO,
...
import warnings
import myldpc.helpers.scipy_helpers
...
cdef BpSparse* Py2BpSparse(pcm):
    ...
    pcm = myldpc.helpers.scipy_helpers.convert_to_binary_sparse(pcm)
...
cdef class BpDecoder(BpDecoderBase):
    ...
    @property
    def oscillation_counts(self) -> np.ndarray:
        """
        Returns the oscillation count for each bit during the last BP decoding.
        The oscillation count is the number of times each bit changed its hard decision
        during the BP iterations.
        
        Returns:
            np.ndarray: A numpy array of length N with oscillation counts per bit.
        """
        out = np.zeros(self.n, dtype=int)
        for i in range(self.n):
            out[i] = self.bpd.osc_count[i]
        return out
