...
namespace ldpc {
    namespace bp {
        ...
        public:
            ...
            // Oscillation count tracking
            std::vector<int> osc_count;
            std::vector<uint8_t> prev_hard_decision;
                    ...
                    iterations(0) //the parity check matrix is passed in by reference
            {
                ...
                // Initialize oscillation count tracking
                this->osc_count.resize(bit_count, 0);
                this->prev_hard_decision.resize(bit_count, 0);
            ...
            void reset_oscillation_counts() {
                std::fill(osc_count.begin(), osc_count.end(), 0);
                std::fill(prev_hard_decision.begin(), prev_hard_decision.end(), 0);
            }

            // Update oscillation counts after hard decision update
            void update_oscillation_counts() {
                for (int i = 0; i < bit_count; i++) {
                    // XOR of previous and current hard decision
                    if (prev_hard_decision[i] != decoding[i]) {
                        osc_count[i]++;
                    }
                    prev_hard_decision[i] = decoding[i];
                }
            }

            // Get oscillation counts
            const std::vector<int>& get_oscillation_counts() const {
                return osc_count;
            }
            ...
            std::vector<uint8_t> decode(std::vector<uint8_t> &input_vector) {
                // Reset oscillation counts at the start of decoding
                reset_oscillation_counts();
            ...
            std::vector<uint8_t> &bp_decode_parallel(std::vector<uint8_t> &syndrome) {
                ...
                for (int it = 1; it <= this->maximum_iterations; it++) {
                    ...
                    // Update oscillation counts after hard decision
                    update_oscillation_counts();
